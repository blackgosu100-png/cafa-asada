...
- 사전 분석에 "생활비유/장비대체 문장" 또는 "창의 제목 각도"가 있으면 제목 후보 최소 2개는 그 방향에서 출발한다. 단, 그 표현이 현재 소스 원문에 직접 있을 때만 쓴다.
`;
...
- lifeAnalogyQuotes가 있으면 bestTitle 후보 1개와 titles 후보 1개 이상은 그 문장의 생활비유/장비대체 느낌을 살린다.
- 저장된 인기글 형식 분석은 발행 시간 추천이 아니라 제목 구조, 감정 결, 댓글 유도 방식을 참고하는 자료다.
- 새벽 시간대 성과가 높게 나와도 업체 홍보글을 새벽에 올리라는 뜻으로 쓰지 않는다. 감정형/상담형/반전형 제목의 결만 빌린다.
...
const HUMANIZE_KOREAN_GUIDE = `
한글 AI 티 제거 하네스:
- 의미 불변: 사실, 수치, 날짜, 고유명사, 직접 인용은 바꾸지 않는다.
- 근거 기반: 소스나 사전 분석에 없는 내용은 자연스럽게 보이더라도 추가하지 않는다.
- 장르 유지: 네이버 카페 글이면 카페 말투 안에서만 자연스럽게 만든다. 칼럼/광고문/보고서처럼 바꾸지 않는다.
- 과윤문 금지: 문장을 너무 매끈하게 정리하지 않는다. 후기의 거친 표현, 머뭇거림, 반전 표현이 클릭 포인트면 살린다.
- 번역투 제거: "~를 통해", "~에 있어서", "~에 대한", "~할 수 있을 것으로 보인다", "가지고 있다" 같은 표현을 피한다.
- 기계적 구조 제거: 첫째/둘째/셋째, 과한 불릿, 균일한 문장 길이, 똑같은 종결어미 반복을 피한다.
- 문두 접속사 남발 금지: 또한, 따라서, 즉, 나아가, 결론적으로로 문장을 이어가지 않는다.
- 형식명사 줄이기: "것", "점", "수", "바", "부분"으로 뭉개지 말고 사람이 말하는 구체어로 쓴다.
- 제목 검수: 제목이 분석 요약문처럼 읽히면 실패다. "키워드+효과"보다 "사람이 툭 올린 말", "궁금한 반전", "댓글 달고 싶은 질문"에 가깝게 고친다.
- 자체검증: 최종 제목/본문을 보고 1) AI 문체가 남았는가 2) 사실이 바뀌었는가 3) 너무 광고 같은가 4) 현재 소스만의 문장인가를 확인하고, 하나라도 실패하면 다시 쓴다.
`;
function buildPrompt(input) {
  ...
  const popularPatternNote = input.popularPatternGuide ? `\n저장된 인기글 형식 분석:\n${input.popularPatternGuide}\n` : "";
  const titleOpinionNote = input.titleOpinion
    ? `\n사람이 넣은 이번 제목 의견:\n${input.titleOpinion}\n`
    : "";
...
2. 사람이 넣은 이번 제목 의견이 있으면 편집장 브리프로 취급한다. 제목 후보 중 최소 2개는 그 의견을 반영하되, 소스에 없는 사실/업종/수치/성능은 만들지 않는다.
3. 제목이 "후기", "좋다네요", "괜찮으세요?", "줄었다는" 같은 분석 요약문 말투면 탈락시킨다.
4. 내부적으로 제목 후보를 30개 생각하고 광고 냄새 나는 후보는 버린 뒤, 클릭 받을 제목 3개만 만든다.
5. bestTitle은 3개 중 가장 조회수가 높을 가능성이 큰 제목으로 고른다. 무난함보다 클릭 욕구를 우선한다.
6. bestReason은 왜 그 제목이 클릭 받을지 짧게 쓴다.
7. draft는 공백 포함 500~800자 사이로 쓴다. 길어도 900자를 넘기지 않는다.
8. 상품명은 본문 중반 이후 자연스럽게 1~2회만 언급한다.
9. 마지막에는 댓글을 부르는 질문을 넣는다.`;
}
...
2. 사전 분석에 lifeAnalogyQuotes 또는 creativeTitleAngles가 있으면 먼저 읽는다. "식세기/로봇청소기/건조기/에어프라이어 이후"처럼 생활을 바꾼 도구와 비교하는 원문이 있으면 bestTitle 후보 1개와 다른 후보 1개 이상은 반드시 그 생활비유에서 출발한다.
3. 사람이 넣은 이번 제목 의견이 있으면 편집장 브리프로 취급한다. 최종 5개 중 최소 2개는 그 의견을 눈에 띄게 반영한다. 하나는 의견을 비교적 직접 반영한 제목, 하나는 더 카페글처럼 우회해서 반영한 제목으로 만든다. 단, 의견이 소스에 없는 사실/업종/수치/성능을 요구하면 그대로 쓰지 말고 안전한 표현 방향만 반영한다.
4. 제목이 식상한지 내부 검수한다. 아래 표현이면 탈락시킨다: "후기", "좋다네요", "괜찮으세요?", "줄었다는", "반대였다는"처럼 분석 요약문에 가까운 말투, 기능명+효과만 붙인 제목, 5개가 모두 같은 어미로 끝나는 제목.
5. 소스에서 제목 소재를 분리해라: 감정핵심 1개, 사람 반응 1개, 비교/비유 문장 1개, 숫자/시간/수량 1개, 대표 이미지/장면 단서 1개, 실제 고충 1개, 믿을 근거 1개, 제품으로 이어질 CTA 1개. 원문에 비교/비유와 사람 반응이 있으면 이미지 속 재료명보다 우선한다.
6. 말투/후킹/CTA를 분리해서 판단한다. 말투와 후킹은 카페 인기글에서 빌리되, CTA는 반드시 주방고수 제품 필요성/문의/링크 확인으로 이어지게 한다.
7. 첨부 이미지가 있으면 이미지 소재 5개도 따로 뽑아라. 예: 인물, 표정, 재료, 기계, 주방 배경, 자막, 용기, 손질 결과물.
8. 내부적으로 제목 후보를 45개 이상 만든다. 이때 최소 10개는 안전한 사실 기반 제목, 최소 10개는 생활비유/반전/사연형 제목, 최소 8개는 사람이 넣은 이번 제목 의견을 반영한 제목으로 만든 뒤 비교한다.
9. 아래 5가지 역할로 최종 제목 5개를 만든다.
...
10. bestTitle은 다섯 후보 중 조회수 가능성이 가장 높은 제목으로 고른다. 단, 사람이 넣은 이번 제목 의견이 있고 사실 검증을 통과한 후보가 충분히 강하면 그 후보를 bestTitle로 우선 고려한다. 소스에 강한 감정핵심/비교문/반전문/생활비유가 있으면 이미지 단서보다 그 문장을 우선하고, 이미지가 그 감정의 근거가 되게 만든다.
...
11. bestReason은 직원이 이해할 수 있게 왜 클릭 받을지 1~2문장으로 쓴다. 추천 이유에는 제목감 원문/감정핵심/생활비유/이미지 연결/제품 CTA 연결/사람 의견 반영 중 최소 2가지를 설명한다.
12. 제목은 16~45자 정도를 우선한다. 너무 긴 설명문이면 줄인다.
13. 분석 결과의 단어를 기계적으로 합치지 마라. "사람 반응 + 매일 손질 + 장면"처럼 명사만 붙은 제목은 실패다.
14. 같은 뜻이면 더 짧고 사람 말 같은 제목을 고른다. 예시 표현은 패턴만 참고하고, 현재 소스에 없는 단어는 제목에 쓰지 않는다. 소스에 실제 사람 반응이 있으면 그 감정형 제목을 bestTitle 최우선 후보로 둔다.
15. 금지: 제목을 상품명으로 시작, 광고 문구, 과장 보장, 소스에 없는 업종/수치 지어내기.
16. 5개 제목은 서로 다른 문장 구조와 끝맺음을 가져야 한다. "바꾼 것", "바꾼 이유", "바꿨습니다" 같은 바꾸다 계열 표현은 전체 후보 중 최대 1개만 쓴다.
17. "추천합니다", "필수템", "전격공개", "최저가", "구매하세요" 같은 광고 단어보다 "왜 밀렸는지", "어디서 막혔는지", "손목이 먼저 보이는지", "몇 통부터 힘든지" 같은 상황 단어를 우선한다.
18. 무난한 기능요약 제목만 5개 나오면 실패다. 최소 2개는 '어? 이건 뭐지?' 싶은 비유형/반전형이어야 한다.`;
}
...
스키마:
{
  ...
  "lifeAnalogyQuotes": ["생활비유/장비대체 문장. 식세기, 로봇청소기, 건조기, 에어프라이어처럼 생활을 바꾼 도구와 비교하는 원문 표현만. 없으면 '현재 소스에 생활비유 원문 없음'이라고 적음"],
  "creativeTitleAngles": ["창의 제목 각도. 원문 근거 안에서 만들 수 있는 비유형/반전형/사연형 제목 방향. 예: 채칼계 로봇청소기 같은 생활 변화급 만족, 단 현재 소스에 없는 비유는 금지"],
...
7. 생활비유/장비대체 문장 10점: 원문에 식세기, 로봇청소기, 건조기, 에어프라이어처럼 생활을 바꾼 도구와 비교하는 문장이 있으면 lifeAnalogyQuotes에 반드시 그대로 적는다. 이런 문장은 기능 키워드보다 제목 우선순위가 높다.
8. 창의 제목 각도 10점: creativeTitleAngles에는 원문 근거 안에서 만들 수 있는 비유형/반전형/사연형 방향을 적는다. 예시 비유를 새 사실처럼 만들지 말고, 원문에 있는 비유만 살린다.
9. 감정반응 10점: 현재 원문에 실제로 있는 사람 반응만 잡는다. 예시 단어를 새 사실처럼 끌어오지 않는다.
10. 이미지연결 10점: 사진/영상에 보이는 장면이 원문핵심을 어떻게 증명하는지 잡는다. 보이는 장면만으로 원문에 없는 성능이나 반응을 만들지 않는다.
11. 제품CTA연결 5점: 댓글/문의/링크 확인/제품 필요성으로 이어질 포인트를 잡는다.
12. hookScores는 감정강도, 구체성, 반전성, 제목화가능성, 제품연결성을 기준으로 100점 만점으로 적고, 80점 이상 문장은 titlePriority 상위에 둔다.
13. topHook에는 hookScores 중 가장 높은 문장 1개를 구조화해서 넣는다. score가 80점 이상이면 mustUseInTitle은 true다.
14. reviewerNotes에는 "기능 키워드보다 더 센 문장", "제목에서 놓치면 손해인 문장", "원문 의미를 바꾸면 위험한 문장"을 적는다.
15. titlePriority 1순위는 lifeAnalogyQuotes에 실제 원문 표현이 있으면 그 표현, 없으면 topHook.quote에서 가져온다. 2순위는 원문핵심/감정핵심, 3순위는 이미지 단서, 4순위는 제품 연결 포인트로 둔다.
16. verifiedFacts에는 최소 5개 이상, titleSafeFacts에는 최소 3개 이상 적는다.
17. titleMustUse와 titleSafeFacts에는 현재 소스에 실제로 있는 단어와 검증 사실만 넣는다.
18. 제목에서 오해될 수 있는 숫자/기간은 titleSafeFacts에 넣지 말고 cautionFlags에 넣는다. 예: 제품 사용 기간이 아닌 기존 작업 기간 "1년"은 "1년 사용"으로 오해되므로 titleSafeFacts에서 제외한다.
19. titleAvoid와 cautionFlags에는 이미지 표면만 설명하거나 현재 소스에 없는 과거 예시 단어를 끌어오는 방향, 원문 의미를 뒤집을 위험이 있는 표현을 적는다.`;
}
...
function parseCafeMenuUrl(rawUrl) {
  ...
  if (/\/cafes\/\d+\/members\//.test(parsed.pathname)) {
    throw new Error("회원 작성글 URL입니다. URL 자동 수집에는 /menus/숫자가 들어간 게시판 URL을 넣어 주세요. 회원 글은 해당 화면에서 목록을 복사해 아래 붙여넣기 분석을 사용해 주세요.");
  }
