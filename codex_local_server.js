...
업소용 제목 추가 공식:
1. 현장사진형: "돈까스집 주방에서 제일 먼저 바꾼 것(사진첨부)"
2. 상황반전형: "피크시간에 밀리던 이유가 양배추였네요"
3. 업종궁금형: "치킨집 샐러드 미리 썰면 진짜 티 나나요?"
4. 짧은정석형: "돈까스집 주방의 정석"
5. 사장님사연형: "가산동 돈까스집 사장님이 양배추 때문에 바꾼 것"
...
- 앞으로는 효과를 제목에서 다 말하지 말고, 사장님이 무엇을 바꿨는지 궁금하게 남긴다.
- 더 좋은 방향: "가산동 돈까스집 사장님이 양배추 때문에 바꾼 것", "반찬가게 어머님이 새벽마다 제일 힘들어한 일", "2년차 샐러드가게 할아버지가 매일 하는거".
- 제목에서 전후 효과를 모두 공개하는 문장보다, 실제 매장/사람 + 매일 하던 일 + 바꾼 것을 숨기는 제목을 우선 추천한다.
...
2. bestTitle은 무난한 제목이 아니라 조회수 가능성이 가장 높은 제목으로 고른다. 특히 "실제 매장/사람 + 매일 하던 일 + 무엇을 바꿨는지 숨김" 구조를 우선한다.
...
${analysisNote}
작성 지시:
1. 먼저 소스에서 제목 소재를 분리해라: 감정핵심 1개, 사람 반응 1개, 숫자/시간/수량 1개, 대표 이미지/장면 단서 1개, 실제 고충 1개, 믿을 근거 1개, 제품으로 이어질 CTA 1개. 원문에 숫자와 사람 반응이 있으면 이미지 속 재료명보다 우선한다.
2. 말투/후킹/CTA를 분리해서 판단한다. 말투와 후킹은 카페 인기글에서 빌리되, CTA는 반드시 주방고수 제품 필요성/문의/링크 확인으로 이어지게 한다.
3. 첨부 이미지가 있으면 이미지 소재 5개도 따로 뽑아라. 예: 인물, 표정, 재료, 기계, 주방 배경, 자막, 용기, 손질 결과물.
4. 내부적으로 제목 후보를 25개 이상 만든다.
5. 아래 5가지 역할로 최종 제목 5개를 만든다.
...
6. bestTitle은 다섯 후보 중 조회수 가능성이 가장 높은 제목으로 고른다. 단, 소스에 강한 감정핵심이 있으면 이미지 단서보다 감정핵심을 우선하고, 이미지가 그 감정의 근거가 되게 만든다.
   - 가장 우선할 구조: "실제 매장/사람 + 매일 하던 일 + 무엇을 바꿨는지 숨김".
   - 예: "가산동 돈까스집 사장님이 양배추 때문에 바꾼 것".
...
7. bestReason은 직원이 이해할 수 있게 왜 클릭 받을지 1~2문장으로 쓴다. 추천 이유에는 감정핵심, 이미지 연결, 제품 CTA 연결 중 최소 2가지를 설명한다.
8. 제목은 16~45자 정도를 우선한다. 너무 긴 설명문이면 줄인다.
9. 분석 결과의 단어를 기계적으로 합치지 마라. "사람 반응 + 매일 손질 + 장면"처럼 명사만 붙은 제목은 실패다.
10. 같은 뜻이면 더 짧고 사람 말 같은 제목을 고른다. 예시 표현은 패턴만 참고하고, 현재 소스에 없는 단어는 제목에 쓰지 않는다. 소스에 실제 사람 반응이 있으면 그 감정형 제목을 bestTitle 최우선 후보로 둔다.
11. 금지: 제목을 상품명으로 시작, 광고 문구, 과장 보장, 소스에 없는 업종/수치 지어내기.
12. "추천합니다", "필수템", "전격공개", "최저가", "구매하세요" 같은 광고 단어보다 "왜 밀렸는지", "무엇을 바꿨는지", "어디서 막혔는지" 같은 상황 단어를 우선한다.`;
}
...
3. 원문핵심 35점: 실제 후기 문장에 있는 숫자, 기간, 재료량, 반복 업무, 고충을 우선한다.
...
6. 감정반응 15점: 현재 원문에 실제로 있는 사람 반응만 잡는다. 예시 단어를 새 사실처럼 끌어오지 않는다.
7. 이미지연결 15점: 사진/영상에 보이는 장면이 원문핵심을 어떻게 증명하는지 잡는다. 보이는 장면만으로 원문에 없는 성능이나 반응을 만들지 않는다.
8. 제품CTA연결 10점: 댓글/문의/링크 확인/제품 필요성으로 이어질 포인트를 잡는다.
9. verifiedFacts에는 최소 5개 이상, titleSafeFacts에는 최소 3개 이상 적는다.
10. titleMustUse와 titleSafeFacts에는 현재 소스에 실제로 있는 단어와 검증 사실만 넣는다.
11. 제목에서 오해될 수 있는 숫자/기간은 titleSafeFacts에 넣지 말고 cautionFlags에 넣는다. 예: 제품 사용 기간이 아닌 기존 작업 기간 "1년"은 "1년 사용"으로 오해되므로 titleSafeFacts에서 제외한다.
12. titleAvoid와 cautionFlags에는 이미지 표면만 설명하거나 현재 소스에 없는 과거 예시 단어를 끌어오는 방향, 원문 의미를 뒤집을 위험이 있는 표현을 적는다.`;
}
